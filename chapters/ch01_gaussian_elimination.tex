...
\begin{itemize}
    ...
    \item The \textbf{growth factor}\index{growth factor} $\rho$: the single number that controls stability
...
Given an invertible matrix \gls{mat} $\in \mathbb{R}^{n\times n}$ and a vector \gls{vec} $\in \mathbb{R}^n$, Gaussian elimination solves $A\mathbf{x} = \mathbf{b}$ in two stages:
...
For \gls{lu factorization}, the computed $\hat{\bm{L}}, \hat{\bm{U}}$ satisfy
\[
...
\begin{itemize}
    ...
    \item \textbf{Partial gls{partial pivoting}}: Choose $i^* = \argmax_{i\ge k} |A_{ik}|$. Swap rows $k \leftrightarrow i^*$. Cost: $O(n^2)$ comparisons (negligible).
...
For complete gls{complete pivoting}, $\rho \le n^{1/2} (2 \cdot 3^{1/2} \cdots n^{1/(n-1)})^{1/2} \sim c n^{1/2} (\log n)^{1/2}$.
\end{theorem}

This proved complete gls{complete pivoting} is stable. But it's \emph{slow}. Partial gls{partial pivoting} is what everyone uses.
...
function growth_factor(A; pivoting=:partial)
    ...
    elseif pivoting == :partial
        # Partial gls{partial pivoting} (row swaps)
        for k = 1:n-1
            # Find pivot
...
Empirical results: for $n = 20$, partial pivoting gives $\mu \approx 10^3$, while complete gls{complete pivoting} gives $\mu \approx 1$.
\end{solution}

\begin{solution}
\textbf{Exercise 2 (Condition number comparison):} For a matrix $A$ with condition number\index{condition number} $\kappa(A) = 10^{12}$, the forward error bound is:
...
\begin{exercise}
Implement complete gls{complete pivoting} in Julia and compare growth factors on the Wilkinson matrix for $n=20, 50, 100$, \cite{urschel2025numerical}.
\end{exercise}
